...
- 思路1： 哈希思想: 时间--$O(n)$， 空间--$O(n)$
- 思路2： 交换思想，数组本身做哈希， 时间- $O(n)$， 空间 $O(1)$

### 2. 不修改数组找出重复的数字 -- 未找到OJ

- 题目： 在一个长度为 n+1 的数组中的所有数字都在 1-n范围内， 所以数组中至少有一个数字是重复的。 请找出数组中任意一个重复的数字，但不能修改输入的数组。

- 思路： 空间复杂度 O(1)， Partition 思想
