...
思想： 根据数组中两个元素关键字的比较结果来对换这两个元素在数组中的位置。

### 1. 冒泡排序

> 思想: 假设数组长为n, 先从后往前(或从前往后) 两两比较相邻元素的值,若为逆序,则交换它们,直到序列比较完； 然后再从 0-n-2
> - 从后往前冒泡： 在 n-1-0范围内，从后往前两两比较，若为逆序，则交换； 然后再从 n-2-0 范围内比较，...
> - 从前往后冒泡： 在 0-n-1 范围内，从前往后两两比较，若为逆序，则交换； 然后从 1-n-1 范围内比较，...
...
> - 思想： 在待排序表$L[1...n]$ 中任意取一个元素 pivot 作为基准， 通过一趟排序将数组划分为独立的两部分 $L[1 ... k-1]$ 和 $L[1...k+1]$， 使得 $L[1...k-1]$ 中所有元素小于pivot， $L[k+1...n]$ 中所有元素大于或等于 pivot, 则pivot放在了其最终位置 L[k] 上。然后分别递归地对两个子表重复上述操作，直至每部分内只有一个元素或空为止，此时所有元素都在其最终的位置上。
> - 优化1： 经典快排一次只将一个元素放置到其最终的位置，如果我们可以参考荷兰国旗问题，一次将与该元素相同的所有元素放到最终位置，这样节省下来常数时间。
> - 经典快排的问题： 划分可能不均匀， 这种情况很容易造成 $O(n^2)$ 
> - 优化2 -- 随机快排： 采用随机选一个数，然后将该数与最后一个元素交换，然后再进行快排， 这是最常用的排序。
...
- 最大堆中要满足：父节点比子节点大于或等于，根节点是最大元素。
- 最小堆中满足： 父节点比子节点要小于或等于， 根节点是最小元素。
...
- 对于近乎有序的数组， 可能存在后面数组中的最小元素大于前面数组中最大元素，因此，对于近乎有序的情况，归并排序的效果会比较好。
- 当子数组中元素较少时， 采用直接插入排序效果会好一些，这是因为在有序数组中，插入排序的时间复杂度为O(n)。
